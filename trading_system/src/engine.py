"""Adaptive strategy, paper execution and hard risk controls."""
...
class RiskGate:
    def __init__(self,cfg): self.cfg=cfg;self.peak_equity=None;self.day_start_equity=None;self.day=time.strftime("%Y-%m-%d");self.kill_switch=False
...
class AdaptiveStrategy:
    """Conservative multi-factor signal; profitability is not guaranteed."""
    def score(self,candles,market):
        closes=[float(x["c"]) for x in candles if x.get("c") is not None]
        if len(closes)<60:return {"action":"HOLD","confidence":0.0,"reason":"warming up"}
        def ema(vals,n):
            k=2/(n+1);out=vals[0]
            for v in vals[1:]:out=v*k+out*(1-k)
            return out
        e20,e50,e100=ema(closes,20),ema(closes,50),ema(closes,100)
        diffs=[b-a for a,b in zip(closes[-15:-1],closes[-14:])];g=[max(d,0) for d in diffs];l=[max(-d,0) for d in diffs]
        rsi=100-100/(1+(statistics.fmean(g)/max(statistics.fmean(l),1e-12)))
        returns=[math.log(b/a) for a,b in zip(closes[-31:-1],closes[-30:]) if a>0 and b>0];vol=statistics.pstdev(returns)*math.sqrt(365*24*4) if len(returns)>2 else 0
        bid,ask=float(market.get("bid") or 0),float(market.get("ask") or 0);price=float(market.get("mid") or market.get("mark") or closes[-1]);spread=((ask-bid)/price*10000) if bid>0 and ask>0 and price>0 else 999
        trend=1 if e20>e50>e100 else -1 if e20<e50<e100 else 0;momentum=1 if rsi>55 else -1 if rsi<45 else 0;funding=float(market.get("funding_rate") or 0);fb=-1 if funding>0.0005 else 1 if funding<-0.0005 else 0
        score=.50*trend+.30*momentum+.20*fb;confidence=min(.97,.50+abs(score)*.45);action="LONG" if score>=.55 else "SHORT" if score<=-.55 else "HOLD"
        return {"action":action,"confidence":confidence,"score":score,"rsi":rsi,"ema20":e20,"ema50":e50,"ema100":e100,"volatility":vol,"spread_bps":spread,"funding_rate":funding,"price":price}

class PaperBroker:
    def __init__(self):self.positions={};self.cash=0.0;self.trades=deque(maxlen=500)
    def mark(self,equity):self.cash=equity
    def execute(self,symbol,side,size,price):
        signed=size if side=="LONG" else -size;self.positions[symbol]=self.positions.get(symbol,0)+signed;self.trades.append({"ts":time.time(),"symbol":symbol,"side":side,"size":size,"price":price,"mode":"PAPER"})
        return self.trades[-1]

class TradingEngine:
    def __init__(self,client,cfg=None):
        self.client=client;self.cfg=cfg or BotConfig();self.risk=RiskGate(self.cfg);self.strategy=AdaptiveStrategy();self.paper=PaperBroker();self.running=False;self.started_at=None;self.last_cycle=0;self.last_error=None;self.latest={};self._task=None;self.events=deque(maxlen=300);self.equity_series=deque(maxlen=500)
    ...
    async def cycle(self):
        ...
        account=self.client.account_snapshot();equity=float(account.get("equity") or 0);self.paper.mark(equity)
        ...
        for item in snapshots:
            ...
            if ok:
                size=self._size(equity,s)
                if self.cfg.live:self._assert_live_ready();await self.client.place_market_order(int(item["id"]),s["action"],size,self.cfg.max_leverage);self.log("TRADE","live order submitted",symbol=item["symbol"],side=s["action"],size=size)
                else:self.paper.execute(item["symbol"],s["action"],size,float(s["price"]));self.log("PAPER","paper order simulated",symbol=item["symbol"],side=s["action"],size=size)
        self.latest={"markets":snapshots,"account":account,"paper_trades":list(self.paper.trades),"ts":time.time()};self.last_cycle=time.time()
    def _assert_live_ready(self):
        if not self.client.authenticated:raise RuntimeError("Live execution blocked: Perpl account is not authenticated")
        if not self.client.api_authenticated:raise RuntimeError("Live execution blocked: Perpl API credentials unavailable")
