...
app=FastAPI(title="Adaptive Perpl Trading System",version="1.0.1")
...
@app.get("/api/status")
def status():return {**engine.status(),"exchange":"Perpl","backend":"online","authenticated":client.authenticated,"timestamp":time.time()}
@app.get("/api/config")
def config():
...
async def start_bot():await engine.start();return engine.status()
...
@app.get("/api/wallet")
def wallet():return JSONResponse({"mode":"wallet-controlled","deposit_supported":True,"withdrawal_supported":True,"api_key_withdrawal":False,"message":"Withdrawals/transfers-out remain wallet-controlled."})
