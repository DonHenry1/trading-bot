...
app=FastAPI(title="Adaptive Perpl Trading System",version="1.1.0")
...
@app.get("/api/status")
def status():return {**engine.status(),"exchange":"Perpl","backend":"online","authenticated":client.authenticated,"api_credentials_present":client.api_authenticated,"timestamp":time.time()}
@app.get("/api/readiness")
def readiness():
    """Non-trading readiness gates. Never places an order or moves funds."""
    ...
    account=client.verify_account() if client.api_authenticated and client.account_id else {"ok":False,"reason":"Perpl account credentials/account ID not configured"}
    balance_ok=False
    if account.get("ok"):
        snap=client.account_snapshot(); balance_ok=float(snap.get("balance") or 0)>0
    gates={"wallet_public_address":bool(client.wallet_address),"perpl_authentication":bool(account.get("ok")),"market_data":market_ok,"account_balance":balance_ok,"paper_execution":True,"risk_controls":not engine.risk.kill_switch,"live_execution":False}
    return {"ready_for_live":all(gates.values()) and os.getenv("PERPL_LIVE_TRADING","false").lower()=="true", "gates":gates,"account":account,"market_reason":market_reason,"live_trading_env":os.getenv("PERPL_LIVE_TRADING","false").lower()=="true","message":"Live execution remains locked until all gates pass and live trading is explicitly enabled."}
@app.get("/api/config")
def config():
...
async def start_bot():
    # Starting the engine is allowed in paper mode. Live mode additionally requires every readiness gate.
    if engine.cfg.live:
        r=readiness()
...
@app.get("/api/wallet")
def wallet():return JSONResponse({"mode":"wallet-controlled","wallet_address":client.wallet_address or None,"deposit":{"available":True,"method":"Use the Perpl wallet interface"},"withdrawal":{"available":True,"method":"Use the Perpl wallet interface","bot_initiated":False},"private_keys_required_by_bot":False,"message":"The bot does not initiate withdrawals or hold your seed phrase/private wallet key. Deposits and withdrawals remain wallet-controlled."})
